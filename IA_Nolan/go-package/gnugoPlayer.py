...
class myPlayer(PlayerInterface):
    ...
    def __init__(self, level=10):
        ...
        self._gnugo = GnuGo.GnuGo(Goban.Board._BOARDSIZE, level)
    ...
    def getPlayerMove(self, alea=False, my_move=''):
        if self._board.is_game_over():
            # print("Referee told me to play but the game is over!")
            return "PASS"
        #  gets the legal moves from Goban (just to write them)
        board_moves = [Goban.Board.flat_to_name(
            m) for m in self._board.legal_moves()]
        # print("Board Legal Moves for player " +
        #      Goban.Board.player_name(self._board._nextPlayer))
        (ok, legal) = self._gnugo.query("all_legal " +
                                        Goban.Board.player_name(self._board._nextPlayer))
